...
//                        municipalityController.getMunicipalityIdByMunicipalityName(jsonObj.get("municipalityName").getAsString());
    ...
    @RequestMapping(value = "/municipality/{municipalityTaxId}/tax-schedule-data", method=RequestMethod.PUT, produces=MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> updateDataForMunicipality(@PathVariable String municipalityTaxId, @RequestBody MunicipalityTaxData municipalityUpdatedData) {
        try {
            boolean has_tax_schedule = false;
            Optional<MunicipalityTaxData> municipalityData = municipalityTaxService.fetchDataById(municipalityTaxId);
            if (municipalityData.isEmpty()) {
                // Return no record found error
                GenericErrorResponse errorResponse = new GenericErrorResponse("404", "No record found for this ID", "");
                ResponseEntity<GenericErrorResponse> response = new ResponseEntity<>(errorResponse, HttpStatus.NOT_FOUND);
                return response;
            }
            else {
                // Only the tax rate, start date and end date can be updated
                municipalityTaxService.saveMunicipalityTaxData(municipalityTaxId,
                        municipalityData.get().getMunicipalityId(),
                        municipalityData.get().getTaxScheduleId(),
                        municipalityData.get().getMunicipalityName(),
                        municipalityUpdatedData.getTaxValue(),
                        municipalityUpdatedData.getStartDate(),
                        municipalityUpdatedData.getEndDate(),
                        municipalityData.get().getTaxSchedule());
                SuccessResponse successResponse = new SuccessResponse(202, "Updated successfully",
                        municipalityTaxId, 1);
                return new ResponseEntity<>(successResponse, HttpStatus.ACCEPTED);
            }
        } catch (Exception e) {
